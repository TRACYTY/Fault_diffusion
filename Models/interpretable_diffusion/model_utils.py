...
class LocalAttentionAdapter(nn.Module):
    def __init__(self, feature_size, seq_length, n_heads=8, window_size=5):
        super().__init__()
        self.feature_size = feature_size  # 使用 feature_size 而非 d_model
        self.n_heads = n_heads
        self.window_size = window_size
        self.attn = nn.MultiheadAttention(feature_size, n_heads, dropout=0.1)
    ...
    def forward(self, x, t):
        b, seq_len, d = x.shape
        assert d == self.feature_size, f"Expected feature dimension {self.feature_size}, but got {d}"
        x_padded = F.pad(x, (0, 0, self.window_size // 2, self.window_size // 2))
        x_windows = x_padded.unfold(1, self.window_size, 1)
        x_windows = x_windows.transpose(2, 3).reshape(b * seq_len, self.window_size, d)
        attn_output, _ = self.attn(x_windows, x_windows, x_windows)
        attn_output = attn_output[:, self.window_size // 2, :].reshape(b, seq_len, d)
        x_local = self.norm(x + self.dropout(self.proj(attn_output)))
        return x_local