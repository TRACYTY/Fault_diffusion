...
from Models.interpretable_diffusion.model_utils import default, identity, extract, LocalAttentionAdapter
...
class Diffusion_TS(nn.Module):
    def __init__(
        ...
        self.n_embd =d_model  # 1024
        # print(f"Init - seq_length: {seq_length},.Concurrent feature_size: {feature_size}, n_embd: {self.n_embd}")
    ...
    #     """启用微调模式，添加局部注意力适配器并冻结预训练参数"""
    #     self.is_finetuning = True
    #     self.local_adapter = LocalAttentionAdapter(feature_size=self.feature_size, seq_length=self.seq_length)
    #     for param in self.model.parameters():
    #         param.requires_grad = False
    # def enable_finetuning(self):
    #     """启用微调模式，添加两个局部注意力适配器并冻结预训练参数"""
    #     self.is_finetuning = True
    #     self.local_adapters = nn.ModuleList([
    #         LocalAttentionAdapter(self.n_embd, self.seq_length)
    #         for _ in range(self.num_local_layers)  # 为两层解码器各创建一个
    #     ])
    #     for param in self.model.parameters():
    #         param.requires_grad = False
    def enable_finetuning(self):
        self.is_finetuning = True
        self.local_adapters = nn.ModuleList([
            LocalAttentionAdapter(self.n_embd, self.seq_length, n_heads=8, window_size=5)
    ...
    def freeze_parameters(self, freeze_encoder=True, freeze_decoder=False):
        for param in self.model.parameters():
            param.requires_grad = True
        if freeze_encoder:
    ...
    # def finetune_forward(self, x, **kwargs):
    #     t = kwargs.get('t', torch.randint(0, self.num_timesteps, (x.shape[0],), device=x.device).long())
    #     trend, season = self.model(x, t, padding_masks=kwargs.get('padding_masks', None))
    #     model_output = trend + season
    #     if self.is_finetuning and self.local_adapter is not None:
    #         model_output = self.local_adapter(model_output, t)
    #     return model_output  # 返回模型输出，而不是损失
    def finetune_forward(self, x, **kwargs):
        t = kwargs.get('t', torch.randint(0, self.num_timesteps, (x.shape[0],), device=x.device).long())
        padding_masks = kwargs.get('padding_masks', None)
        # print(f"Input x shape: {x.shape}")  # 调试输入形状
        # Embedding and Encoder (frozen)
        emb = self.model.emb(x)
        # print(f"Embedded shape: {emb.shape}")
        inp_enc = self.model.pos_enc(emb)
        enc_cond = self.model.encoder(inp_enc, t, padding_masks=padding_masks)

        # Decoder with two layered local attention
        inp_dec = self.model.pos_dec(emb)
        decoder_output = inp_dec
        local_output_accum = 0  # 累积前一层的局部注意力输出
        mean_outputs = []
        season_output = torch.zeros_like(inp_dec)  # 初始化 season
        trend_output = torch.zeros_like(x)  # 初始化 trend，与输入 x 的形状一致

        # 遍历解码器块
        for i, block in enumerate(self.model.decoder.blocks):
            # 解码器层前向
            residual, mean, trend, season = block(decoder_output, enc_cond, t, mask=padding_masks)
            # 累加输出
            # print(f"Block {i} - residual: {residual.shape}, season: {season.shape}")  # 调试
            decoder_output = residual
            mean_outputs.append(mean)
            trend_output = trend_output + trend
            season_output = season_output + season

            # 如果在微调模式，添加局部注意力
            if self.is_finetuning and i < self.num_local_layers:
                local_input = decoder_output + local_output_accum  # [batch_size, seq_length, n_embd]
                # print(f"Local input {i}: {local_input.shape}")  # 调试
                local_output = self.local_adapters[i](local_input, t)  # [batch_size, seq_length, n_embd]
                local_output_accum = local_output
                decoder_output = decoder_output + local_output

        # 组合最终输出，与 Transformer.forward 一致
        res = self.model.inverse(decoder_output)
        res_m = torch.mean(res, dim=1, keepdim=True)
        mean = torch.cat(mean_outputs, dim=1)  # 拼接所有层的 mean
        season_final = self.model.combine_s(season_output.transpose(1, 2)).transpose(1, 2)
        trend_final = self.model.combine_m(mean) + res_m + trend_output
        # print(f"Output shape: {(trend_final + season_final).shape}")  # 调试
        # 返回最终输出（可以选择 trend_final + season_final 或 res）
        return trend_final + season_final
    ...
    def return_components(self, x, t: int):
        b, c, n, device, feature_size, = *x.shape, x.device, self.feature_size
        assert n == feature_size, f'number of variable must be {feature_size}'
        t = torch.tensor([t]).repeat(b).to(device)
        x = self.q_sample(x, t)
    ...
    def sample_infill(self, shape, target, partial_mask=None, clip_denoised=True, model_kwargs=None):
        ...
        img = torch.randn(shape, device=device)
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='conditional sampling loop time step', total=self.num_timesteps):
